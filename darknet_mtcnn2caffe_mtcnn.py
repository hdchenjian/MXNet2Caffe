...
parser.add_argument('--cf-prototxt', type=str, default='model_caffe/darknet_mtcnn/det3_permute.prototxt')
parser.add_argument('--cf-model',        type=str, default='model_caffe/darknet_mtcnn/det3_permute.caffemodel')
...
for line in f.readlines():
    if 'name: "' not in line:
        continue
    if 'ONet' in line: continue
    if 'conv' not in line and 'relu' not in line and 'pool' not in line and 'connect' not in line:
        continue
...
weight_file = open('/home/luyao/git/cnn/model/mtcnn_final.weights', 'rb')
...
for i_key,key_i in enumerate(network_keys):
    try:
        print key_i, weight_index
        if 'data' == key_i or '_scale' in key_i:
            continue
        elif 'conv' in key_i or 'connect' in key_i:
            print(type(net.params[key_i][0].data), net.params[key_i][0].data.shape,
                  type(net.params[key_i][0].data.shape), get_weight_num(net.params[key_i][0].data.shape))
	    net.params[key_i][0].data.flat = darknet_weight[weight_index : weight_index + get_weight_num(net.params[key_i][0].data.shape)]
            ...
            print(key_i, type(net.params[key_i][0].data), net.params[key_i][0].data.shape, type(net.params[key_i][0].data.shape))
            net.params[key_i][0].data.flat = darknet_weight[weight_index : weight_index + get_weight_num(net.params[key_i][0].data.shape)]
            weight_index += get_weight_num(net.params[key_i][0].data.shape)
        elif 'pool' in key_i:
            continue
